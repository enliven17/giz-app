...
cd "$ROOT/core"
case "$(uname -s)" in
 Darwin) HOST_LIBRARY=target/debug/libgizu_stored_signer_core.dylib; NDK_HOST=darwin-x86_64 ;;
 Linux) HOST_LIBRARY=target/debug/libgizu_stored_signer_core.so; NDK_HOST=linux-x86_64 ;;
