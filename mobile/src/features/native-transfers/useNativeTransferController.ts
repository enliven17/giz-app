...
export function transferProposal(index: string, recipient: string, amount: string): string {
  if (
  ...
  return JSON.stringify({
    ...
    chainId: 10143,
    transfers: [{ accountIndex: Number(index), to: recipient, valueWei: value.toString() }],
  });
}
export function useNativeTransferController(service: NativeTransfers | null) {
  ...
  async function run(refresh: boolean) {
    ...
    if (!refresh) {
      try {
        proposal = transferProposal(account, recipient.trim(), amount.trim());
      } catch {
        setMessage(
          "Enter account 0–15, a full recipient address, and an amount above 0 up to 0.1 MON.",
        );
        return;
      }
    }
