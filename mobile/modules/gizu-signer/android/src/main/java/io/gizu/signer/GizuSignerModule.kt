...
class GizuSignerModule : Module() {
  ...
  private data class NativeAccessResult(val address: String? = null, val proofs: List<ProbeProof> = emptyList())
  private fun openAccess(promise: Promise, walletOnly: Boolean) {
      scope.launch {
              ...
              val result = withTimeout(120_000) {
                ...
                if (create) {
                  val request = JSONObject().put("challenge", b64(random()))
                    ...
                    .put("user", JSONObject().put("id", b64(random())).put("name", if (walletOnly) "gizu-testnet-wallet" else "gizu-native-test").put("displayName", if (walletOnly) "Gizu testnet wallet" else "Gizu native test"))
                ...
                try {
                  ...
                  // Credential UI may briefly own focus; only return public access in foreground.
                  withTimeout(10_000) { while (!activity.hasWindowFocus()) { delay(50); ensureActive() } }
                  // Only this private native path accepts secret bytes. No bridge export.
                  if (walletOnly) NativeAccessResult(address = deriveWalletAddress(prf))
                  else NativeAccessResult(proofs = runNativeProbe(prf, random().joinToString("") { "%02x".format(it) }))
                } finally { prf.fill(0) }
              }
              val publicResults: Any = if (walletOnly) mapOf("address" to result.address, "accountIndex" to 0, "chainId" to 10143) else result.proofs.map { mapOf("accountIndex" to it.accountIndex.toInt(), "address" to it.address, "message" to it.message, "signature" to it.signatureHex) }
        ...
        dialog = AlertDialog.Builder(activity)
          .setTitle(if (walletOnly) "Open Gizu testnet wallet" else "Native compatibility test")
          .setMessage(if (walletOnly) "Create or open a passkey to view Account 0 on Monad testnet. No transaction or message will be signed. This opens a local wallet view, not a backend login. Creation may require two prompts." else "Use an unfunded test passkey. Approve 16 fixed test signatures across accounts 0–15. No transactions, login or spending. Keys stay in native memory and are released after this test. Creation may require two prompts.")
          .setPositiveButton("Open existing") { _, _ -> start(false) }
          .setNeutralButton(if (walletOnly) "Create passkey" else "Create test passkey") { _, _ -> start(true) }
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("openNativeProbe") { promise: Promise -> openAccess(promise, false) }
    AsyncFunction("openWallet") { promise: Promise -> openAccess(promise, true) }
