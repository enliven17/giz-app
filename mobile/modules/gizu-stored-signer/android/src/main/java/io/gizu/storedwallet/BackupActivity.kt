...
internal object BackupHost {
  ...
  suspend fun open(activity: Activity, restore: Boolean) {
    try { suspendCancellableCoroutine<Unit> { result ->
    check(completion == null)
    val id = UUID.randomUUID().toString()
    token = id; completion = result
    result.invokeOnCancellation { activity.runOnUiThread { close(false) } }
    try { activity.startActivity(Intent(activity, BackupActivity::class.java)
      .putExtra("token", id).putExtra("restore", restore)) }
  ...
  fun close(success: Boolean) {
    ...
    completion = null; token = null
    val current = screen; screen = null
    fun complete() {
      if (result?.isActive == true) { if (success) result.resume(Unit) else result.cancel() }
    }
    if (current != null) {
      current.cancelWork(); current.finish()
      closing = CoroutineScope(Dispatchers.Main.immediate).launch {
        current.awaitWork(); complete()
      }
    } else complete()
  }
}
...
class BackupActivity : Activity() {
  ...
  internal fun cancelWork() { workJob.cancel() }
  internal suspend fun awaitWork() { workJob.join() }
  ...
  override fun onCreate(state: Bundle?) {
    ...
    if (state != null || intent.getStringExtra("token") != BackupHost.token || BackupHost.token == null) {
      finish(); return
    }
    ...
    message = TextView(this).apply { textSize = 20f; setTextColor(0xffe0e8e2.toInt()) }
    message.text = if (restore) "Restore Gizu wallet\n\nChoose your encrypted backup. You need the original passkey. Recovery restores wallet accounts, not transaction history."
      else "Back up your Gizu wallet\n\nSave the encrypted file, then reopen it to verify recovery. You need BOTH this file and your original passkey to recover your wallet. Keep them available independently of this phone. You will confirm your passkey twice."
    action = Button(this).apply {
      text = if (restore) "Choose backup" else "Save encrypted backup"
      filterTouchesWhenObscured = true
      setOnClickListener { if (!busy) { if (restore || saved) choose() else save() } }
    }
    layout.addView(message); layout.addView(action)
    layout.addView(Button(this).apply { text = "Cancel"; filterTouchesWhenObscured = true
      setOnClickListener { BackupHost.close(false) } })
    val scroll = ScrollView(this).apply { addView(layout); setBackgroundColor(0xff080d09.toInt()) }
    val padding = (24 * resources.displayMetrics.density).toInt()
    scroll.setOnApplyWindowInsetsListener { _, insets ->
      @Suppress("DEPRECATION")
      val top = if (Build.VERSION.SDK_INT >= 30) insets.getInsets(android.view.WindowInsets.Type.systemBars()).top else insets.systemWindowInsetTop
      @Suppress("DEPRECATION")
      val bottom = if (Build.VERSION.SDK_INT >= 30) insets.getInsets(android.view.WindowInsets.Type.systemBars()).bottom else insets.systemWindowInsetBottom
  ...
  private suspend fun prf(credential: StoredPasskey): ByteArray {
    unlocked(); external = true
    try {
      val bytes = PasskeyGate(this).recoveryPrf(credential)
      try {
        withTimeout(10000) { while (!hasWindowFocus()) delay(50) }
        unlocked(); currentCoroutineContext().ensureActive()
        return bytes
      } catch (error: Throwable) { bytes.fill(0); throw error }
    } finally { external = false }
  }
  private fun work(block: suspend () -> Unit) {
    busy = true; action.isEnabled = false
    scope.launch {
      try { unlocked(); block() }
      catch (_: CancellationException) { BackupHost.close(false) }
      catch (_: Exception) {
        message.text = "Backup could not be verified. Use the original passkey and an unchanged Gizu backup. Your current wallet has not been replaced. You can retry or cancel."
      } finally { busy = false; action.isEnabled = true }
    }
  }
  private fun save() = work {
    val credential = withContext(Dispatchers.IO) { store.load().use { it.credential } }
    val key = prf(credential)
    try {
      encrypted = withContext(Dispatchers.IO) { store.load().use { record ->
    ...
    }, 1) } catch (error: Exception) { external = false; throw error }
  }
  private fun choose() {
    external = true
    try { startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply {
      addCategory(Intent.CATEGORY_OPENABLE); type = "*/*"
    }, 2) } catch (_: Exception) { external = false; BackupHost.close(false) }
  }
  @Deprecated("Platform document picker callback")
  override fun onActivityResult(request: Int, result: Int, data: Intent?) {
    super.onActivityResult(request, result, data)
    external = false
    if (result != RESULT_OK || data?.data == null) { BackupHost.close(false); return }
    val uri = data.data!!
    work {
      if (request == 1) {
        ...
        encrypted?.fill(0); encrypted = null; saved = true
        message.text = "Backup saved. Now reopen that file and confirm your original passkey so Gizu can verify all wallet accounts."
        action.text = "Open saved backup to verify"
      } else if (request == 2) verify(uri)
  ...
  private suspend fun verify(uri: Uri) {
    ...
    val bytes = withContext(Dispatchers.IO) {
      contentResolver.openInputStream(uri)!!.use { input ->
        ...
        while (count < buffer.size) {
          ...
          if (read == -1) break
          check(read > 0); count += read
        }
        require(count in 1..65536); buffer.copyOf(count)
      }
    }
    val credential = BackupCodec.credential(bytes)
    if (restore) check(withContext(Dispatchers.IO) { store.state()["status"] in listOf("absent", "recoveryRequired") })
    else withContext(Dispatchers.IO) { store.load().use {
      check(it.credential.credentialId.contentEquals(credential.credentialId))
      check(it.credential.publicKeyX.contentEquals(credential.publicKeyX) && it.credential.publicKeyY.contentEquals(credential.publicKeyY))
    } }
    val key = prf(credential)
    try {
      withContext(Dispatchers.IO) {
        BackupCodec.decrypt(bytes, key).use { restored ->
          ...
          if (!restore) store.load().use { current ->
    ...
    unlocked(); BackupHost.close(true)
  }
  override fun onPause() { super.onPause(); if (!external && !isFinishing) BackupHost.close(false) }
  @Deprecated("Platform back")
  override fun onBackPressed() { BackupHost.close(false) }
  override fun onDestroy() {
    if (observingLock) { unregisterReceiver(lockReceiver); observingLock = false }
    scope.cancel(); encrypted?.fill(0); encrypted = null
