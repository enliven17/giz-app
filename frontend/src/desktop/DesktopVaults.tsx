  ...
  return (
    <div className="mx-auto flex min-h-0 w-full max-w-[1520px] flex-1 flex-col px-14 pb-6 pt-7">
      ...
      <div className="mt-5 flex shrink-0 items-center gap-3">
        ...
        <div className="flex gap-2">
          {FILTERS.map((f) => (
            <button
              key={f}
              onClick={() => setRisk(f)}
              className={`rounded-2xl px-5 py-3 font-mono text-[11px] uppercase tracking-widest ${
        ...
        <button className="glass-soft flex h-[46px] w-[46px] items-center justify-center rounded-2xl text-white/45">
          <SlidersHorizontal size={16} />
        </button>
      </div>

      <div className="mt-5 grid min-h-0 flex-1 grid-cols-4 grid-rows-2 gap-5">
        {list.map((v, i) => (
          <VaultCard
            key={v.id}
            vault={v}
